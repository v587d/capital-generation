...
def main() -> int:
    ...
    # 子命令统一接受 --db (放子命令后; 顶层 --db 兼容放前)
    parent = argparse.ArgumentParser(add_help=False)
    parent.add_argument("--db", default=None, help=argparse.SUPPRESS)

    p = sub.add_parser("sync", parents=[parent], help="全量/增量同步 (需 THS key)")
    p.set_defaults(fn=cmd_sync)
    p = sub.add_parser("update", parents=[parent], help="增量补最近交易日 (需 THS key)")
    p.set_defaults(fn=cmd_update)
    p = sub.add_parser("validate", parents=[parent], help="8 项质量校验")
    p.add_argument("--json", action="store_true")
    p.set_defaults(fn=cmd_validate)
    p = sub.add_parser("rebuild", parents=[parent], help="重建视图与因子 (禁删 raw)")
    p.set_defaults(fn=cmd_rebuild)
    p = sub.add_parser("status", parents=[parent], help="库状态 (schema_version/行数/最大日期)")
    p.add_argument("--json", action="store_true")
    p.set_defaults(fn=cmd_status)
    p = sub.add_parser("describe", parents=[parent], help="schema 描述 (JSON)")
    p.set_defaults(fn=cmd_describe)
    p = sub.add_parser("query", parents=[parent], help="只读 SQL 查询")
    p.add_argument("--sql", required=True)
    p.add_argument("--limit", type=int, default=None)
    p.add_argument("--json", action="store_true")
    p.set_defaults(fn=cmd_query)
    p = sub.add_parser("import-parquet", parents=[parent], help="离线导入本地 Parquet")
