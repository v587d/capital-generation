...
export function registerDataCollectorTools(ctx: Context, hub: DataCollectorHub, diagnostics?: DataCollectorDiagnostics): void {
  ...
  const registrations = [
    ...
    tool(
      'list_capabilities',
      '列出当前已注册的数据能力：短 capability 名、用途描述、参数 input_schema、输出结构 output_schema 与是否支持分页。capability 是模型侧唯一标识（短小、稳定、全局唯一）；宿主内部的数据源路由字段不在此返回。request_data 的 capability 必须以本工具返回的值为准，不要自行编造。',
      jsonObject(),
      ...
      }, ['capability', 'description', 'input_schema', 'output_schema', 'paginated']) },
