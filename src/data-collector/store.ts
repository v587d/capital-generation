...
export function normalizeWorkspaceRelativePath(value: string): string {
  if (typeof value !== 'string') throw new DatasetStoreError('workspace_path_invalid', 'path must be a string')
  const trimmed = value.trim()
  if (trimmed.length === 0 || trimmed.length > 512) throw new DatasetStoreError('workspace_path_invalid', 'path must be 1-512 characters')
  if (trimmed.includes('\\')) throw new DatasetStoreError('workspace_path_invalid', 'path must use forward slashes')
  if (isAbsolutePath(trimmed)) throw new DatasetStoreError('workspace_path_invalid', 'path must be relative to the session workspace')
  const segments = trimmed.split('/').filter((segment) => segment.length > 0)
  if (segments.length === 0) throw new DatasetStoreError('workspace_path_invalid', 'path must name a file inside the workspace')
  for (const segment of segments) {
    if (segment === '.' || segment === '..') throw new DatasetStoreError('workspace_path_invalid', 'path must not contain "." or ".." segments')
  }
...
export class WorkspaceDatasetStore {
  ...
  async readPresentationRows(input: {
    ...
    signal?: AbortSignal
    maxRows?: number
  }): Promise<{ ref: DatasetRef; rows: unknown[] }> {
    const stored = await this.requireStoredRef(input.dataset_id, input.session, input.signal)
    const { session_scope_id: _scope, row_key: rowKey, ...ref } = stored
    const raw = await this.readRaw(ref, input.session, input.signal, rowKey)
    if (raw.kind !== 'rows') {
  ...
  async writeWorkspaceFiles(input: {
    ...
    const written: Array<{ name: string; path: string; absolutePath: string }> = []
