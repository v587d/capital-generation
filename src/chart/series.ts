...
export interface ChartOhlcPayload {
  label: string
  data: Array<{ time: number | string; open: number; high: number; low: number; close: number }>
}

export interface ChartVolumePayload {
  label: string
  data: Array<{ time: number | string; value: number; direction?: 'up' | 'down' }>
}
...
export interface ChartPayload {
  ...
  markers: Array<{ time: number | string; text: string; position: MarkerPosition }>
 ...
 * 自己指定路径，而不是静默取第一个。
 */
export function extractRows(value: unknown): { rows: unknown[]; shape: string } {
  ...
  for (const key of preferred) {
    ...
    if (Array.isArray(candidate) && candidate.length > 0) return { rows: candidate, shape: key }
  }
  const arrayKeys = Object.keys(value).filter((key) => Array.isArray(value[key]) && (value[key] as unknown[]).length > 0)
  if (arrayKeys.length === 1) return { rows: value[arrayKeys[0]!] as unknown[], shape: arrayKeys[0]! }
...
function collectFields(records: ReadonlyArray<Record<string, unknown>>): string[] {
  const seen: string[] = []
  ...
  for (const record of records) {
    for (const key of Object.keys(record)) {
      ...
      set.add(key)
      seen.push(key)
      if (seen.length >= 200) return seen
    }
  }
  return seen
}

function assertFields(fields: ReadonlyArray<string | undefined>, available: readonly string[]): void {
  const missing = [...new Set(fields.filter((field): field is string => field !== undefined))].filter((field) => !available.includes(field))
...
function toNumber(value: unknown): number | undefined {
  ...
  if (typeof value === 'string') {
    ...
    if (text.length === 0 || text.length > 32) return undefined
 ...
 */
export function parseTimeValue(value: unknown): ParsedTime | undefined {
  ...
  if (/^\d+$/.test(text)) return parseTimeValue(Number(text))
...
export function buildChartPayload(input: BuildChartPayloadInput): ChartPayload {
  const { spec } = input
  ...
  if (spec.axis === 'time' && spec.timeField !== undefined) assertFields([spec.timeField], available)
  for (const series of spec.series) assertFields([series.field], available)
  if (spec.ohlc) assertFields([spec.ohlc.open, spec.ohlc.high, spec.ohlc.low, spec.ohlc.close], available)
  if (spec.volume) assertFields([spec.volume.field], available)

  // ── 1. 时间轴判定 ─────────────────────────────────────────────────────────
  // 显式给了 x 但值不像时间（分类数据）时，自动降级为分类轴并用 x 的取值做刻度标签，
  // 而不是报错让模型改口——"用 x 画柱状图"是最自然的表达。
  const axisField = spec.timeField
  let axis: 'time' | 'index' = axisField === undefined ? 'index' : 'time'
  ...
  if (skippedRows > 0) warnings.push(`${skippedRows} row(s) skipped: the time field is missing or unparseable`)
  if (prepared.length === 0) {
    // OHLC 图形必须给可解析的时间列：它不是"降级"问题，而是这张图根本画不出来。
    // 错误必须直接把模型引到能用的 kind 上，而不是让它反复试同一个 spec。
    if ((OHLC_KINDS as readonly string[]).includes(spec.kind)) {
  ...
  if (ordered.some((row, index) => row !== prepared[index])) warnings.push('rows were reordered ascending by time (the chart library requires an ordered axis)')
  ...
  if (unique.length !== ordered.length) warnings.push(`${ordered.length - unique.length} duplicate timestamp(s) collapsed`)
  ...
  if (spec.range) {
    ...
    if (spec.range.to !== undefined && to === undefined) throw new ChartError('chart_spec_invalid', 'spec.range.to is not a recognizable date')
    filtered = unique.filter((row) => (from === undefined || row.time.seconds >= from) && (to === undefined || row.time.seconds <= to))
    if (filtered.length === 0) throw new ChartError('chart_no_rows', 'spec.range excludes every row')
  }
  ...
  const displayTime = (row: PreparedRow): number | string => (useBusinessDay ? String(row.time.display) : row.time.seconds)

  // ── 6. 子序列 ────────────────────────────────────────────────────────────
  const series: ChartSeriesPayload[] = []
  for (const item of spec.series.slice(0, MAX_CHART_SERIES)) {
    const data: Array<{ time: number | string; value: number }> = []
    let dropped = 0
    ...
    if (data.length === 0) throw new ChartError('chart_field_not_found', `field "${item.field}" has no numeric value in the selected range`, { field: item.field })
    if (dropped > 0) warnings.push(`series "${item.label}": ${dropped} point(s) dropped (non-numeric or missing)`)
    series.push({ id: item.field, label: item.label, type: item.type, data })
  }

  let ohlc: ChartOhlcPayload | undefined
  if (spec.ohlc) {
    const data: ChartOhlcPayload['data'] = []
    let dropped = 0
    ...
    for (const row of kept) {
      ...
      if (high < low || open > high || open < low || close > high || close < low) {
        inconsistent += 1
        if (inconsistentExamples.length < 3) inconsistentExamples.push(String(row.time.display))
      }
      data.push({ time: displayTime(row), open, high, low, close })
    }
    if (data.length === 0) throw new ChartError('chart_field_not_found', 'the OHLC fields have no numeric values in the selected range')
    if (dropped > 0) warnings.push(`OHLC: ${dropped} 行因字段缺失被丢弃`)
    if (inconsistent > 0) {
  ...
  let volume: ChartVolumePayload | undefined
  const volumeSpec = spec.volume
  if (volumeSpec) {
    const data: ChartVolumePayload['data'] = []
    let dropped = 0
    for (const row of kept) {
      ...
      if (spec.ohlc) {
        const open = toNumber(row.raw[spec.ohlc.open])
        const close = toNumber(row.raw[spec.ohlc.close])
        if (open !== undefined && close !== undefined) direction = close >= open ? 'up' : 'down'
      }
      data.push({ time: displayTime(row), value, ...(direction === undefined ? {} : { direction }) })
    }
    if (data.length === 0) throw new ChartError('chart_field_not_found', `field "${volumeSpec.field}" has no numeric value in the selected range`, { field: volumeSpec.field })
    if (dropped > 0) warnings.push(`volume: ${dropped} point(s) dropped (non-numeric or missing)`)
  ...
  if (axis === 'index') for (const row of kept) labels[String(row.time.seconds)] = row.indexLabel
...
function primarySeriesValues(rows: ReadonlyArray<PreparedRow>, spec: NormalizedChartSpec): number[] {
...
function nearestRow(rows: ReadonlyArray<PreparedRow>, keptSeconds: readonly number[], target: number): PreparedRow | undefined {
  ...
  for (const index of candidates) {
    if (Math.abs(keptSeconds[index]! - target) < Math.abs(keptSeconds[chosen]! - target)) chosen = index
  }
  return rows[chosen]
}
