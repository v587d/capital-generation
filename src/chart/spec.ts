...
export const CHART_KINDS: readonly ChartKind[] = ['line', 'area', 'column', 'candlestick', 'bar']
/**
 ...
 */
export const OHLC_KINDS: readonly ChartKind[] = ['candlestick', 'bar']
export const CHART_SERIES_TYPES: readonly ChartSeriesType[] = ['line', 'area', 'column']
export const MARKER_POSITIONS: readonly MarkerPosition[] = ['aboveBar', 'belowBar', 'inBar']
...
const SERIES_TYPE_ALIASES: Record<string, ChartSeriesType> = { histogram: 'column' }
...
function coerceObjectList(value: unknown): unknown[] | undefined {
...
function readFieldName(value: unknown, where: string): string {
  if (typeof value !== 'string') throw new ChartError('chart_spec_invalid', `${where} must be a string field name`)
...
function readOptionalFieldName(value: unknown, where: string): string | undefined {
  if (value === undefined || value === null || value === '') return undefined
  return readFieldName(value, where)
}

/** 取第一个出现的别名键；用于容忍模型用 x / time / time_field 等等价说法。 */
function pickAlias(source: Record<string, unknown>, names: readonly string[]): unknown {
...
function readKind(value: unknown): ChartKind {
  ...
  const kind = (KIND_ALIASES[raw] ?? raw) as ChartKind
...
function readSeries(value: unknown): ChartSeriesSpec[] {
  ...
  return list.map((entry, index): ChartSeriesSpec => {
    if (typeof entry === 'string') return { field: readFieldName(entry, `spec.series[${index}]`), label: entry.trim(), type: 'line' }
    if (!isRecord(entry)) throw new ChartError('chart_spec_invalid', `spec.series[${index}] must be a field name or an object`)
    ...
    if (entry.type !== undefined) {
      ...
      const resolved = (SERIES_TYPE_ALIASES[raw] ?? raw) as ChartSeriesType
      if (!(CHART_SERIES_TYPES as readonly string[]).includes(resolved)) {
...
function readOhlc(value: unknown): ChartOhlcSpec {
...
function readVolume(value: unknown): { field: string } | undefined {
  if (value === undefined || value === null || value === '') return undefined
  if (typeof value === 'string') return { field: readFieldName(value, 'spec.volume') }
  const volume = coerceObject(value)
  if (!volume) throw new ChartError('chart_spec_invalid', 'spec.volume must be a field name or { field }')
  const field = readOptionalFieldName(volume.field ?? volume.column, 'spec.volume.field')
  if (field === undefined) throw new ChartError('chart_spec_invalid', 'spec.volume requires a field name')
  return { field }
}

function readMarkers(value: unknown): ChartMarkerSpec[] {
  if (value === undefined || value === null) return []
  const list = coerceObjectList(value)
  if (list === undefined) throw new ChartError('chart_spec_invalid', 'spec.markers must be an array of { time|index, text, position? }')
  ...
  return list.map((entry, index): ChartMarkerSpec => {
    if (!isRecord(entry)) throw new ChartError('chart_spec_invalid', `spec.markers[${index}] must be an object`)
    ...
    if (!(MARKER_POSITIONS as readonly string[]).includes(rawPosition)) {
    ...
    const marker: ChartMarkerSpec = { text, position: rawPosition as MarkerPosition }
...
function readRange(value: unknown): { from?: string | number; to?: string | number } | undefined {
  if (value === undefined || value === null || value === '') return undefined
  const range = coerceObject(value)
  if (!range) throw new ChartError('chart_spec_invalid', 'spec.range must be an object { from?, to? }')
  const from = pickAlias(range, ['from', 'start'])
  const to = pickAlias(range, ['to', 'end'])
  const read = (raw: unknown, where: string): string | number | undefined => {
  ...
  if (result.from === undefined && result.to === undefined) return undefined
  return result
}

export function normalizeChartSpec(raw: unknown): NormalizedChartSpec {
  ...
  if ((OHLC_KINDS as readonly string[]).includes(kind)) {
