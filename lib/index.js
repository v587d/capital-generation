...
import { registerDataCollectorTools } from './data-collector/tools.js';
...
import { createChartEventPublisher } from './chart/events.js';
 ...
 */
export function resolveLocalFetchConfig(config) {
    const source = config ?? {};
 ...
 */
export function resolveUserCustomizationSection(customPersona) {
...
export function apply(ctx, config) {
            ...
            try {
                ...
                if (credentials?.resolve) {
                    for (const ref of [effectiveConfig.fuyaoCredentialRef || 'FUYAO_API_KEY']) {
                        ...
                        if (resolved?.value)
                            return { present: true, source: resolved.source ?? `credentials(${ref})` };
                    }
                }
                const processLike = globalThis.process;
                const fuyaoCredentialRef = effectiveConfig.fuyaoCredentialRef || 'FUYAO_API_KEY';
                if (processLike?.env?.[fuyaoCredentialRef])
                    return { present: true, source: `env(${fuyaoCredentialRef})` };
    ...
    const rawCtx = ctx;
    const chartEventContext = {
        get: (name) => rawCtx.get(name),
        ...
        on: (event, listener) => {
            if (typeof rawCtx.on !== 'function')
                return undefined;
            return rawCtx.on.call(ctx, event, (...args) => { listener(...args); });
        },
        onTurnStopping: (listener) => {
            if (typeof rawCtx.on !== 'function')
                return undefined;
            try {
                return rawCtx.on.call(ctx, 'agent/turn-stopping', (...args) => { listener(...args); });
            }
            catch {
                return undefined;
            }
    ...
    try {
        if (rawCtx.root !== undefined) {
            Object.defineProperty(chartEventContext, 'root', { get: () => rawCtx.root, enumerable: true });
        }
    }
    ...
    try {
        if (typeof rawCtx.effect === 'function') {
            Object.defineProperty(chartEventContext, 'effect', {
                value: (callback, label) => rawCtx.effect.call(ctx, callback, label),
                enumerable: true,
            });
        }
    }
    ...
    const client = createAnySearchClient(retriever.baseURL || undefined, async () => {
        try {
            const credentials = ctx.get('credentials');
    ...
    // 是本机直连出网——开关必须管得到它们，否则用户关掉后插件仍发本机 HTTP（同意被旁路）。
    // 关闭时工具**仍在注册面**但每次调用响亮失败（DISABLED），不静默消失；共用出口校验与超时/大小上限。
    registerSourceTools(ctx, {
        enabled: localFetchConfig.enabled,
        timeoutMs: localFetchConfig.timeoutMs,
        maxBytes: localFetchConfig.maxBytes,
        maxContentChars: localFetchConfig.maxContentChars,
        maxRedirects: localFetchConfig.maxRedirects,
