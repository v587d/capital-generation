...
function validateInputSchema(inputSchema, values, allowed) {
    const schema = inputSchema;
    const required = Array.isArray(schema.required) ? schema.required : [];
    for (const name of required) {
        const value = values[String(name)];
        if (value === undefined || value === null || value === '')
            throw new Error(`missing required parameter: ${String(name)}`);
    }
    for (const name of Object.keys(values)) {
        ...
        if (descriptor?.enum && !descriptor.enum.includes(value))
            throw new Error(`parameter ${name} has an invalid value`);
    }
}
...
function source(name, capability, description, input_schema, output_schema, path, allowed, baseUrl, resolveApiKey, paginated = false) {
    const schema = {
        capability,
        name,
        source: 'api:fuyao',
...
const envelopeData = (item) => ({ type: 'object', properties: { timestamp: { type: 'integer' }, total: { type: 'integer' }, item }, additionalProperties: true });
...
function validateEnvelopeFields(data, requiredFields) {
...
export function createFuyaoRestSources(resolveApiKey, baseUrl = DEFAULT_BASE_URL) {
    ...
    return [
        source('get_meta_tickers_search', 'ticker_search', '按名称、代码或简称检索并消歧为完整 thscode（禁止自行拼接交易所后缀，先消歧再请求业务数据）。q 必填；exchange 取值 SH/SZ/BJ；asset_type 逗号分隔。', objectSchema({
        ...
        }, ['q']), searchOutput, '/api/meta/tickers/search', ['q', 'exchange', 'asset_type', 'limit'], cleanBaseUrl, resolveApiKey),
        source('get_a_share_prices_snapshot', 'quote', '获取 A 股行情快照。两种模式：①指定标的：thscodes 传逗号分隔的完整代码（如 600519.SH,000001.SZ），忽略分页；②全市场遍历：省略 thscodes，用 limit/offset。快照不含股票名称。', objectSchema({
        ...
        source('get_a_share_prices_historical', 'history', '获取单只标的历史日 K 线。thscode 必须是单只（不可逗号）；interval 当前固定支持 1d（日线）；start/end 为毫秒级 Unix 时间戳（Asia/Shanghai），end >= start，跨度不超过 10 年；adjust 默认 forward（前复权）。', objectSchema({
        ...
        }, ['thscode', 'interval', 'start', 'end']), klineOutput, '/api/a-share/prices/historical', ['thscode', 'interval', 'start', 'end', 'adjust', 'offset'], cleanBaseUrl, resolveApiKey, true),
        source('get_a_share_calendar_trading_days', 'trading_calendar', '获取近一年（365 天）A 股交易日历；无需任何参数。返回 item[]：date_ms 为交易日零点毫秒戳，date 为 YYYYMMDD 字符串。', objectSchema({}), calendarOutput, '/api/a-share/calendar/trading-days', [], cleanBaseUrl, resolveApiKey),
    ];
}
