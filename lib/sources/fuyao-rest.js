 ...
 */
function jsonParam(value, name, kind, maxLength = MAX_PARAM_LENGTH) {
    ...
    if (!ok)
        fail(`parameter ${name} must be a JSON ${kind} string`);
    const encoded = canonicalJson(parsed);
    if (encoded.length > maxLength)
        fail(`parameter ${name} exceeds maximum length ${maxLength}`);
    return encoded;
}
function optionalJsonParam(value, name, kind) {
    if (value === undefined)
        return undefined;
...
function assertKnownParameters(values, allowed) {
...
function text(value, name, maxLength = MAX_TEXT_LENGTH) {
    ...
    if (trimmed.length === 0)
        fail(`parameter ${name} must not be empty`);
...
function integer(value, name, options = {}) {
    ...
    if (options.minimum !== undefined && value < options.minimum)
        fail(`parameter ${name} must be >= ${options.minimum}`);
    if (options.maximum !== undefined && value > options.maximum)
        fail(`parameter ${name} must be <= ${options.maximum}`);
    return value;
}
function requiredInteger(value, name, options = {}) {
    if (value === undefined || value === null)
        fail(`missing required parameter: ${name}`);
    return integer(value, name, options);
}
function numberValue(value, name, options = {}) {
...
function booleanValue(value, name) {
    if (typeof value !== 'boolean')
        fail(`parameter ${name} must be a boolean`);
    return value;
}
function enumValue(value, name, allowed, normalize = (input) => input) {
    const raw = normalize(text(value, name, 64));
    if (!allowed.includes(raw))
        fail(`parameter ${name} has an invalid value: ${raw} (allowed: ${allowed.join(', ')})`);
    return raw;
}
function requiredEnum(value, name, allowed, normalize) {
    if (value === undefined || value === null || value === '')
        fail(`missing required parameter: ${name}`);
...
function dateString(value, name) {
    ...
    if (!matched)
        fail(`parameter ${name} must be a date in YYYY-MM-DD form`);
    ...
    if (Number(year) < 1990)
        fail(`parameter ${name} is outside the supported date range`);
    return raw;
}
/** 财务指标报告期：`yyyy-1`..`yyyy-4`（1 一季报 / 2 中报 / 3 三季报 / 4 年报）。 */
function reportPeriod(value, name) {
    const raw = text(value, name, 16);
    const matched = /^(\d{4})-([1-4])$/.exec(raw);
    if (!matched)
        fail(`parameter ${name} must be a report period like 2025-1 (yyyy-1 .. yyyy-4)`);
    if (Number(matched[1]) < 1990)
        fail(`parameter ${name} is outside the supported period range`);
    return raw;
}
...
/** 规范化单个指数代码（允许同花顺 `.TI`）。 */
function indexCodeToken(value, name) {
    const raw = text(value, name, 32).toUpperCase();
 ...
 */
function codeList(value, name, options = {}) {
    ...
    if (tokens.length === 0)
        fail(`parameter ${name} must contain at least one thscode`);
...
function enumList(value, name, allowed) {
    ...
    for (const token of raw.split(',').map((item) => item.trim().toLowerCase()).filter((item) => item.length > 0)) {
        if (!allowed.includes(token))
            fail(`parameter ${name} has an invalid value: ${token} (allowed: ${allowed.join(', ')})`);
        if (!unique.includes(token))
            unique.push(token);
    }
    if (unique.length === 0)
        fail(`parameter ${name} must contain at least one value`);
    return unique.join(',');
}
...
function queryParams(params, allowed) {
    ...
    for (const name of allowed) {
        ...
        if (encoded.length > MAX_PARAM_LENGTH)
            fail(`parameter ${name} exceeds maximum length`);
        search.set(name, encoded);
    }
    return search;
}
async function callFuyao(baseUrl, apiKey, path, params, signal, outputSchema) {
    const query = params.toString();
    const response = await fetch(`${baseUrl}${path}${query ? `?${query}` : ''}`, { headers: { 'X-api-key': apiKey, Accept: 'application/json' }, signal });
    if (!response.ok)
        throw new Error(`Fuyao HTTP error ${response.status}`);
    ...
    if (envelope.code !== 0)
        throw new Error(`Fuyao API error ${envelope.code ?? 'unknown'}: ${envelope.message ?? 'request failed'}${envelope.request_id ? ` (request_id: ${envelope.request_id})` : ''}`);
    return { data: envelope.data ?? null, schema: outputSchema };
}
function matchesField(value, spec) {
...
function matchesFields(record, fields) {
...
function buildGuard(spec) {
    return (data) => {
        ...
        if (spec.custom && !spec.data && !spec.item)
            return spec.custom(data);
        if (!isRecord(data))
            return false;
        if (!optionalInteger(data.timestamp) || !optionalInteger(data.total))
            return false;
        if (spec.data && !matchesFields(data, spec.data))
            return false;
        if (spec.custom && !spec.custom(data))
            return false;
        if (spec.item) {
            const rows = data[spec.itemKey ?? 'item'];
            if (!Array.isArray(rows))
                return false;
            if (!rows.every((row) => isRecord(row) && matchesFields(row, spec.item)))
                return false;
        }
        return true;
    };
...
function validateAbilities(data) {
    ...
    return data.abilities.every((ability) => {
        if (!isRecord(ability))
            return false;
        if (typeof ability.ability !== 'string' || !Array.isArray(ability.indicators))
            return false;
...
const envelopeData = (item) => ({ type: 'object', properties: { timestamp: nullableInteger, total: nullableInteger, item }, additionalProperties: true });
...
const tickerFields = { thscode: 'text', ticker: 'text?', name: 'text?', exchange: 'text?', asset_type: 'text?', currency: 'text?', list_date: 'text?', end_date: 'text?', last_trade_date: 'text?', last_delivery_date: 'text?' };
const valuationFields = { thscode: 'text', ticker: 'text?', name: 'text?', pe_ttm: 'number?', pe_mrq: 'number?', pb_mrq: 'number?', ps_ttm: 'number?', pcf_ttm: 'number?' };
const auctionFields = { thscode: 'text', ticker: 'text?', name: 'text?', auction_price: 'number?', auction_pct: 'number?', auction_volume: 'number?', auction_amount: 'number?', auction_unmatched: 'number?', auction_turnover_pct: 'number?', auction_yesterday_ratio_pct: 'number?', auction_volume_ratio: 'number?', pre_close_price: 'number?', open_price: 'number?', last_price: 'number?', float_market_cap: 'number?' };
const limitUpPoolFields = { thscode: 'text', ticker: 'text?', name: 'text?', is_st: 'boolean?', is_new: 'boolean?', last_price: 'number?', price_change_ratio_pct: 'number?', limit_up_time: 'text?', limit_up_reason: 'text?', continue_day_text: 'text?', continue_day_cnt: 'number?', seal_money: 'number?', max_seal_money: 'number?' };
const ladderFields = { date: 'text', boards: 'object' };
const indexCatalogFields = { thscode: 'text', name: 'text?' };
const indexConstituentFields = { thscode: 'text', ticker: 'text?', name: 'text?' };
const indexQuoteFields = { thscode: 'text', ticker: 'text?', last_price: 'number?', price_change: 'number?', price_change_ratio_pct: 'number?', open_price: 'number?', high_price: 'number?', low_price: 'number?', prev_price: 'number?', volume: 'number?', turnover: 'number?' };
const indexHistoryFields = { date_ms: 'number', open_price: 'number?', high_price: 'number?', low_price: 'number?', close_price: 'number?', volume: 'number?', turnover: 'number?' };
/** 财务三表共有的期间标识字段（严格）与币种（宽容）。 */
const limitDownFields = { thscode: 'text', ticker: 'text?', name: 'text?', last_price: 'number?', price_change_ratio_pct: 'number?', first_limit_time: 'text?', last_limit_time: 'text?', turnover_ratio_pct: 'number?' };
const limitBreakFields = { thscode: 'text', ticker: 'text?', name: 'text?', last_price: 'number?', price_change_ratio_pct: 'number?', open_times: 'number?', turnover_ratio_pct: 'number?', turnover: 'number?' };
/** 飙升榜与热股榜共用字段；heat 是上游原始**字符串**（保留精度，不转 number）。 */
const rankListFields = { thscode: 'text', ticker: 'text?', name: 'text?', rank: 'number?', heat: 'text?', rank_change: 'number?', rank_trend: 'text?' };
const hotStockHistoryFields = { thscode: 'text', ticker: 'text?', name: 'text?', rank: 'number?' };
const rankTrendFields = { thscode: 'text', ticker: 'text?', date: 'text', date_ms: 'number?', rank: 'number?' };
const anomalyFields = { thscode: 'text', stock_name: 'text?', analysis_content: 'text?', keyword_list: 'array?', tag_name: 'text?' };
const dragonTigerStockFields = { thscode: 'text', ticker: 'text?', name: 'text?', concept_list: 'array?', change: 'number?', net_value: 'number?', net_rate: 'number?', hot_rank: 'number?', buy_value: 'number?', sell_value: 'number?', limit_reason: 'text?', range_days: 'number?', org_net_value: 'number?', org_net_rate: 'number?', org_buy_num: 'number?', org_sell_num: 'number?', amount: 'number?', hot_money_net_value: 'number?', hot_money_net_rate: 'number?', hot_money_item_net_value: 'number?', hot_money_item_net_rate: 'number?' };
const auctionBenchmarkFields = { thscode: 'text', ticker: 'text?', name: 'text?', auction_pct: 'number?', tags: 'array?' };
const fundProfileFields = { thscode: 'text', ticker: 'text?', fund_name: 'text?', estab_date: 'number?', company_id: 'text?', mgmt_name: 'text?', manager_name: 'text?', fund_scale: 'number?', unit_nav: 'number?', manager_info: 'array?', trade_rule: 'array?', rate_info: 'array?' };
const fundQuoteFields = { thscode: 'text', ticker: 'text?', last_price: 'number?', open_price: 'number?', high_price: 'number?', low_price: 'number?', prev_price: 'number?', price_change_ratio_pct: 'number?', price_change: 'number?', price_amplitude_ratio_pct: 'number?', volume: 'number?', turnover: 'number?', turnover_ratio_pct: 'number?' };
const fundHistoryFields = { date_ms: 'number', open_price: 'number?', high_price: 'number?', low_price: 'number?', close_price: 'number?', volume: 'number?', turnover: 'number?' };
const fundNavFields = { nav_date: 'number', unit_nav: 'number?', adj_nav: 'number?' };
/**
...
function validateFundReturns(data) {
    if (!isRecord(data) || !Array.isArray(data.item))
        return false;
    return data.item.every((row) => isRecord(row)
        && Object.keys(row).some((key) => FUND_RETURN_PREFIXES.some((prefix) => key.startsWith(prefix))));
}
const fundDrawdownFields = { thscode: 'text', ticker: 'text?', week: 'number?', month: 'number?', tmonth: 'number?', hyear: 'number?', year: 'number?', twoyear: 'number?', tyear: 'number?', fyear: 'number?', nowyear: 'number?', now: 'number?' };
const fundHoldingFields = { thscode: 'text', ticker: 'text?', stock_name: 'text?', hold_ratio: 'number?', asset_type: 'text?', position_capital: 'number?', position_count: 'number?', security_market_value_rate_pct: 'number?', period_increase_rate_pct: 'number?', investment_rank: 'number?', start_date_ms: 'number?', end_date_ms: 'number?', publish_date_ms: 'number?', modify_time_ms: 'number?' };
const fundAssetAllocationFields = { report_date_ms: 'number', stock_ratio_pct: 'number?', bond_ratio_pct: 'number?', deposit_ratio_pct: 'number?', other_ratio_pct: 'number?' };
const fundIndustryAllocationFields = { report_period: 'text', industry_name: 'text?', ratio_pct: 'number?' };
const fundStockHistoryFields = { thscode: 'text', ticker: 'text?', name: 'text?', asset_type: 'text?', hold_ratio: 'number?', market_value: 'number?', period_increase_pct: 'number?', rank: 'number?', report_type: 'text?', end_date_ms: 'number?' };
const fundHolderFields = { merge_scope: 'text', report_date_ms: 'number', ins_position: 'number?', holder_amount: 'number?', avg_holder_share: 'number?', psnl_rate: 'number?', mgmt_staff_hold_rate: 'number?' };
const fundTopHolderFields = { holder_id: 'text', holder_code: 'text?', holder_name: 'text?', holder_type: 'text?', rank: 'number?', hold_share: 'number?', hold_rate_pct: 'number?', report_date_ms: 'number?', publish_date_ms: 'number?' };
const fundManagerFields = { manager_id: 'text', manager_name: 'text?', sex: 'text?', degree: 'text?', company_id: 'text?', company_name: 'text?', resume: 'text?', photo_url: 'text?', annual_return_pct: 'number?', maximum_return_pct: 'number?', radar_comparison: 'array?' };
const fundCompanyFields = { company_id: 'text', company_name: 'text?', company_type: 'text?', established_date_ms: 'number?', fund_count: 'number?', scale: 'number?' };
const fundPerformanceHistoryFields = { date_ms: 'number', rsi_pct: 'number?', donchian_channel: 'number?', track_index_pe_ttm_five_year_percentile: 'number?' };
const fundReportDatesFields = { report_type: 'text', report_type_name: 'text?', start_date_ms: 'number?', end_date_ms: 'number?' };
const fundManagerExperienceFields = { awards: 'json?', heavy_assets: 'json?', investment_history: 'json?' };
const fundManagerStyleFields = { representative_fund_thscode: 'text?', representative_fund_ticker: 'text?', representative_fund_name: 'text?', investment_idea: 'text?', total_fund_scale: 'number?', industry_preferences: 'json?' };
const fundManagerPerformanceFields = { date_ms: 'number', manager_return_pct: 'number?', peer_return_pct: 'number?', benchmark_return_pct: 'number?' };
const fundFinancialDateFields = { start_date_ms: 'number', end_date_ms: 'number', publish_date_ms: 'number?' };
const fundNumberFields = (names) => Object.fromEntries(names.map((name) => [name, 'number?']));
const fundIncomeFields = { ...fundFinancialDateFields, ...fundNumberFields(['income', 'investment_income', 'stock_investment_income', 'bond_investment_income', 'fund_investment_income', 'dividend_income', 'interest_income', 'fair_value_income', 'exchange_income', 'other_income', 'total_income', 'fee', 'total_fee', 'manager_reward', 'custodian_fee', 'transaction_cost', 'tax_surcharge', 'total_profit', 'net_profit']) };
const fundBalanceFields = { ...fundFinancialDateFields, ...fundNumberFields(['total_assets', 'bank_deposit', 'fund_investment', 'stock_investment', 'bond_investment', 'transactional_financial_assets', 'other_assets', 'total_liability', 'other_liability', 'owner_total_equity', 'undistributed_profit', 'liability_and_owner_equity']) };
const fundFinancialIndicatorFields = { ...fundFinancialDateFields, ...fundNumberFields(['distribution_profit', 'current_profit', 'current_income', 'distribution_share_profit', 'average_nav_profit_margin', 'average_share_current_profit', 'share_nav', 'sum_nav_rate', 'asset_nav', 'sum_share_nav', 'nav_rate']) };
const fundDividendFields = { per_ten_cash_before_tax: 'number?', per_ten_cash_after_tax: 'number?', progress: 'text?', publish_date_ms: 'number?', registration_date_ms: 'number?', ex_dividend_date_ms: 'number?', payment_date_ms: 'number?', reinvestment_date_ms: 'number?', profit_base_date_ms: 'number?', in_dividend_date_ms: 'number?' };
const fundOfferingFields = { thscode: 'text', ticker: 'text?', subscription_start_ms: 'number?', subscription_end_ms: 'number?' };
const fundQuotaSummaryFields = { name: 'text', unlimited: 'text?', total_limit: 'text?', total: 'text?', buy: 'text?' };
/** 实测：dimensions/peer_dimensions/resilience/peer_resilience 是**数组**、probabilities/ranges 为 null（文档写 object）。 */
const fundDiagnosticsFields = { thscode: 'text', ticker: 'text?', fund_type: 'text?', peer_code: 'text?', dimensions: 'json?', peer_dimensions: 'json?', probabilities: 'json?', ranges: 'json?', resilience: 'json?', peer_resilience: 'json?' };
const fundBacktestIndicatorFields = { indicator_code: 'text', indicator_name: 'text?', id: 'number?', type: 'number?', value_kind: 'number?', support_operation: 'text?', unit: 'text?', description: 'text?', state_rules: 'text?', create_time: 'text?', update_time: 'text?' };
/** quota/list 是三层嵌套（data[] → sub_tab[] → fund_list[]），没有 item[] 信封。 */
function validateQuotaList(data) {
    if (!Array.isArray(data))
        return false;
    return data.every((category) => {
        if (!isRecord(category))
            return false;
        if (typeof category.name !== 'string' || !Array.isArray(category.sub_tab))
            return false;
        return category.sub_tab.every((subTab) => {
            if (!isRecord(subTab))
                return false;
            if (!Array.isArray(subTab.fund_list))
                return false;
...
function validateIndicatorLine(data) {
    if (!isRecord(data))
        return false;
    if (!Array.isArray(data.time_range))
        return false;
    if (!data.time_range.every((point) => point === null || Number.isSafeInteger(point)))
        return false;
    if (!Array.isArray(data.indexes) || !Array.isArray(data.data))
        return false;
    if (!data.indexes.every((meta) => isRecord(meta) && typeof meta.index_id === 'string'))
        return false;
    return data.data.every((series) => {
        if (!isRecord(series) || typeof series.thscode !== 'string' || !Array.isArray(series.values))
            return false;
...
function validateIndicatorTable(data) {
    if (!isRecord(data))
        return false;
...
const financialNumber = (names) => Object.fromEntries(names.map((name) => [name, 'number?']));
function normalizeQuote(params) {
    ...
    if (params.limit !== undefined)
        normalized.limit = integer(params.limit, 'limit', { minimum: 1 });
    if (params.offset !== undefined)
        normalized.offset = integer(params.offset, 'offset', { minimum: 0 });
    return normalized;
}
/** 时间区间型请求的共用校验：成对出现、end >= start、窗口不超过 10 年。 */
function timeWindow(params, normalized) {
    const { start, end } = params;
    if ((start === undefined) !== (end === undefined))
        fail('parameters start and end must be provided together');
    if (start === undefined || end === undefined)
        return;
    const from = integer(start, 'start', { minimum: 0 });
    const to = integer(end, 'end', { minimum: 0 });
    if (to < from)
        fail('parameter end must be >= start');
    if (to - from > MAX_HISTORY_WINDOW_MS)
        fail('parameter window (end - start) exceeds the 10 year limit');
    normalized.start = from;
    normalized.end = to;
}
function normalizeHistory(params) {
    const normalized = {
    ...
    if (end < start)
        fail('parameter end must be >= start');
    if (end - start > MAX_HISTORY_WINDOW_MS)
        fail('parameter window (end - start) exceeds the 10 year limit');
    normalized.start = start;
    normalized.end = end;
    if (params.adjust !== undefined)
        normalized.adjust = enumValue(params.adjust, 'adjust', ADJUST_MODES, lowerCase);
    if (params.offset !== undefined)
        normalized.offset = integer(params.offset, 'offset', { minimum: 0 });
    return normalized;
}
function normalizeTickerSearch(params) {
    const normalized = { q: requiredText(params.q, 'q') };
    if (params.exchange !== undefined)
        normalized.exchange = enumValue(params.exchange, 'exchange', SEARCH_EXCHANGES, (input) => input.toUpperCase());
    if (params.asset_type !== undefined)
        normalized.asset_type = enumList(params.asset_type, 'asset_type', SEARCH_ASSET_TYPES);
    if (params.limit !== undefined)
        normalized.limit = integer(params.limit, 'limit', { minimum: 1, maximum: 50 });
    return normalized;
}
function normalizeTickerList(params) {
    const normalized = {};
    if (params.asset_type !== undefined)
        normalized.asset_type = enumList(params.asset_type, 'asset_type', CATALOG_ASSET_TYPES);
    if (params.limit !== undefined)
        normalized.limit = integer(params.limit, 'limit', { minimum: 1, maximum: 10000 });
    if (params.offset !== undefined)
        normalized.offset = integer(params.offset, 'offset', { minimum: 0 });
    return normalized;
}
function normalizeCorporateActions(params) {
    const normalized = { thscode: codeToken(requiredText(params.thscode, 'thscode', 32), 'thscode') };
    ...
    if (from !== undefined && to !== undefined && to < from)
        fail('parameter to must be >= from');
    if (from !== undefined)
...
function normalizeFinancial(params) {
    const normalized = {
        thscode: codeToken(requiredText(params.thscode, 'thscode', 32), 'thscode'),
    ...
    if (params.limit !== undefined)
        normalized.limit = integer(params.limit, 'limit', { minimum: 1, maximum: 20 });
    timeWindow(params, normalized);
    return normalized;
}
function normalizeFinancialIndicators(params) {
    return {
        thscode: codeToken(requiredText(params.thscode, 'thscode', 32), 'thscode'),
        report: reportPeriod(requiredText(params.report, 'report', 16), 'report'),
    };
}
function normalizeValuation(params) {
    return { thscodes: codeList(params.thscodes, 'thscodes', { maximum: MAX_BATCH_CODES }) };
}
function normalizeAuction(params) {
    const normalized = { thscodes: codeList(params.thscodes, 'thscodes', { maximum: MAX_BATCH_CODES }) };
    if (params.stage !== undefined)
        normalized.stage = enumValue(params.stage, 'stage', AUCTION_STAGES, lowerCase);
    return normalized;
}
function normalizeLimitUpPool(params) {
    const normalized = {};
    if (params.date_ms !== undefined)
        normalized.date_ms = integer(params.date_ms, 'date_ms', { minimum: 0 });
    if (params.page !== undefined)
        normalized.page = integer(params.page, 'page', { minimum: 1 });
    if (params.size !== undefined)
        normalized.size = integer(params.size, 'size', { minimum: 1, maximum: 200 });
    if (params.sort_field !== undefined)
        normalized.sort_field = enumValue(params.sort_field, 'sort_field', LIMIT_UP_SORT_FIELDS, lowerCase);
    if (params.sort_dir !== undefined)
        normalized.sort_dir = enumValue(params.sort_dir, 'sort_dir', SORT_DIRECTIONS, lowerCase);
    return normalized;
}
function normalizeIndexCatalog(params) {
    const normalized = {};
    if (params.tag !== undefined)
        normalized.tag = enumValue(params.tag, 'tag', INDEX_TAGS, lowerCase);
    return normalized;
}
function normalizeIndexConstituents(params) {
    return { thscode: indexCodeToken(requiredText(params.thscode, 'thscode', 32), 'thscode') };
}
function normalizeIndexQuote(params) {
    return { thscodes: codeList(params.thscodes, 'thscodes', { kind: 'index' }) };
}
/** 基金代码规范化：trim + 大写 + 市场后缀校验（不接受逗号与裸代码）。 */
function fundCodeToken(value, name) {
    const raw = text(value, name, 32).toUpperCase();
...
function identifierToken(value, name) {
    return requiredText(value, name, 64);
}
/** 只带 thscode 的基金端点（基本资料、业绩快照、持仓、持有人等）。 */
function onlyFundCode(params) {
    return { thscode: fundCodeToken(requiredText(params.thscode, 'thscode', 32), 'thscode') };
}
function normalizeFundMarketHistory(params) {
    const normalized = { thscode: fundCodeToken(requiredText(params.thscode, 'thscode', 32), 'thscode') };
    if (params.interval !== undefined)
        normalized.interval = enumValue(params.interval, 'interval', KLINE_INTERVALS, lowerCase);
    const start = requiredInteger(params.start, 'start', { minimum: 0 });
    const end = requiredInteger(params.end, 'end', { minimum: 0 });
    if (end < start)
        fail('parameter end must be >= start');
    if (end - start > MAX_FUND_HISTORY_WINDOW_MS)
        fail('parameter window (end - start) exceeds the 5 year limit for fund market history');
    normalized.start = start;
    normalized.end = end;
    return normalized;
}
function normalizeFundNav(params) {
    const normalized = { thscode: fundCodeToken(requiredText(params.thscode, 'thscode', 32), 'thscode') };
    if (params.range !== undefined)
        normalized.range = enumValue(params.range, 'range', NAV_RANGES, lowerCase);
    if (params.nav_type !== undefined)
        normalized.nav_type = enumValue(params.nav_type, 'nav_type', NAV_TYPES, lowerCase);
    return normalized;
}
function normalizeFundStockHistory(params) {
    return {
...
function normalizeFundHolders(params) {
    const normalized = { thscode: fundCodeToken(requiredText(params.thscode, 'thscode', 32), 'thscode') };
    if (params.merge_scope !== undefined)
        normalized.merge_scope = enumValue(params.merge_scope, 'merge_scope', MERGE_SCOPES, lowerCase);
    return normalized;
}
function normalizeFundTopHolders(params) {
    const normalized = { thscode: fundCodeToken(requiredText(params.thscode, 'thscode', 32), 'thscode') };
    if (params.limit !== undefined)
        normalized.limit = integer(params.limit, 'limit', { minimum: 1, maximum: MAX_TOP_HOLDERS_LIMIT });
    return normalized;
}
function normalizeFundManager(params) {
    return { manager_id: identifierToken(params.manager_id, 'manager_id') };
}
function normalizeFundCompany(params) {
    return { company_id: identifierToken(params.company_id, 'company_id') };
}
/** 报告期查询（bond-history 与 stock-history 同构）。 */
function normalizeFundReportType(params) {
    const normalized = { thscode: fundCodeToken(requiredText(params.thscode, 'thscode', 32), 'thscode') };
    if (params.report_type !== undefined)
        normalized.report_type = text(params.report_type, 'report_type', 32);
    return normalized;
}
function normalizeFundPerformanceHistory(params) {
    const normalized = { thscode: fundCodeToken(requiredText(params.thscode, 'thscode', 32), 'thscode') };
    const start = requiredInteger(params.start, 'start', { minimum: 0 });
    const end = requiredInteger(params.end, 'end', { minimum: 0 });
    if (end < start)
...
function normalizeManagerRange(params) {
    return {
...
function normalizeFundIndicatorLine(params) {
    const indexes = jsonParam(params.indexes, 'indexes', 'array');
    const groups = JSON.parse(indexes);
    if (groups.length === 0)
        fail('parameter indexes must contain at least one group');
    for (const [position, group] of groups.entries()) {
        if (!isRecord(group))
            fail(`parameter indexes group ${position} must be an object`);
...
function normalizeFundIndicatorTable(params) {
    const normalized = {};
    ...
    if (codeSelectors !== undefined)
        normalized.code_selectors = codeSelectors;
    if (indexes !== undefined)
        normalized.indexes = indexes;
    if (pageInfo !== undefined)
        normalized.page_info = pageInfo;
    if (sort !== undefined)
...
function normalizeFundQuotaList(params) {
    const normalized = { tab: jsonParam(params.tab, 'tab', 'array') };
    if (params.buy !== undefined)
        normalized.buy = booleanValue(params.buy, 'buy');
    return normalized;
}
function normalizeFundBacktest(params) {
    return {
        thscode: fundCodeToken(requiredText(params.thscode, 'thscode', 32), 'thscode'),
...
function normalizePoolQuery(params, sortFields) {
    const normalized = {};
    if (params.date_ms !== undefined)
        normalized.date_ms = integer(params.date_ms, 'date_ms', { minimum: 0 });
    if (params.page !== undefined)
    ...
    if (params.size !== undefined)
        normalized.size = integer(params.size, 'size', { minimum: 1, maximum: 200 });
    if (params.sort_field !== undefined)
        normalized.sort_field = enumValue(params.sort_field, 'sort_field', sortFields, lowerCase);
    if (params.sort_dir !== undefined)
        normalized.sort_dir = enumValue(params.sort_dir, 'sort_dir', SORT_DIRECTIONS, lowerCase);
    return normalized;
}
function normalizeLimitDownPool(params) {
    return normalizePoolQuery(params, LIMIT_DOWN_SORT_FIELDS);
}
function normalizeLimitBreakPool(params) {
    return normalizePoolQuery(params, LIMIT_BREAK_SORT_FIELDS);
}
/** 飙升榜 / 热股榜：period 取 day（日/24 小时级）或 hour（小时级）。 */
function normalizeRankList(params) {
    const normalized = {};
    if (params.period !== undefined)
        normalized.period = enumValue(params.period, 'period', RANK_PERIODS, lowerCase);
    return normalized;
}
function normalizeHotStockHistory(params) {
    return { date: dateString(requiredText(params.date, 'date', 32), 'date') };
}
function normalizeHotStockRankTrend(params) {
    ...
    if (startDate > endDate)
        fail('parameter end_date must be >= start_date');
    return {
        thscode: codeToken(requiredText(params.thscode, 'thscode', 32), 'thscode'),
...
function anomalyTagList(value, name) {
    const raw = requiredText(value, name, MAX_TEXT_LENGTH);
    const tokens = raw.split(',').map((token) => token.trim().toUpperCase());
    if (tokens.some((token) => token.length === 0))
        fail(`parameter ${name} must not contain empty tokens`);
    const unique = [];
    for (const token of tokens) {
        if (!ANOMALY_TAGS.includes(token))
            fail(`parameter ${name} has an invalid value: ${token} (allowed: ${ANOMALY_TAGS.join(', ')})`);
        if (!unique.includes(token))
            unique.push(token);
    }
    return unique.join(',');
}
function normalizeAnomalyList(params) {
    const normalized = {};
    if (params.tag_codes !== undefined)
        normalized.tag_codes = anomalyTagList(params.tag_codes, 'tag_codes');
    return normalized;
}
function normalizeAnomalyStock(params) {
    return { thscodes: codeList(params.thscodes, 'thscodes', { maximum: MAX_ANOMALY_CODES }) };
}
function normalizeDragonTiger(params) {
    const normalized = {};
    if (params.board_type !== undefined)
        normalized.board_type = enumValue(params.board_type, 'board_type', DRAGON_TIGER_BOARDS, lowerCase);
    if (params.date !== undefined)
        normalized.date = dateString(params.date, 'date');
    return normalized;
}
function normalizeIndexHistory(params) {
    const normalized = {
...
function endpointDefinitions() {
    return [
        ...
        },
        {
            ...
            allowed: ['subscribe'],
            normalize: (params) => ({ subscribe: requiredEnum(params.subscribe, 'subscribe', OFFERING_SUBSCRIBE, lowerCase) }),
        ...
        },
        {
            ...
            allowed: ['tab'],
            normalize: (params) => ({ tab: jsonParam(params.tab, 'tab', 'array') }),
...
function createSource(definition, baseUrl, resolveApiKey) {
    ...
    return {
        ...
        execute: async (request, signal) => {
            ...
            if (!apiKey)
                throw new Error(`FUYAO_API_KEY is not configured; data source ${name} is unavailable`);
            return callFuyao(baseUrl, apiKey, path, queryParams(params, allowed), signal, outputSchema);
        },
    };
}
export function createFuyaoRestSources(resolveApiKey, baseUrl = DEFAULT_BASE_URL) {
    const cleanBaseUrl = baseUrl.replace(/\/$/, '');
    return endpointDefinitions().map((definition) => createSource(definition, cleanBaseUrl, resolveApiKey));
}
export async function resolveFuyaoApiKey(ctx, credentialRef = 'FUYAO_API_KEY') {
