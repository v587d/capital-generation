...
function resolvePeriod(period, anchor) {
...
export function resolveDataTimeRange(args, now = Date.now()) {
    ...
    if (contract.kind === 'enum_range')
        return { capability, format: 'enum', timezone: zone, params: { [contract.fields[0]]: enumPeriod(contract, args.period) }, warnings: [] };
...
function dataTimeRangeDefinition() {
    return {
        ...
        async execute(args, _exec) { return resolveDataTimeRange(args); },
    };
}
