import { MAX_ITEMS, SourceError, clsTelegraph, cninfoIrm, createSourceTransport, eastmoneyFastNews, eastmoneyReports, eastmoneyStockNews, sinaReports, sseinfoQa, thsEpsForecast, wscnLives, } from './sources.js';
...
function sourceOutput(outcome, recent, tally) {
    const items = outcome.items.slice(0, SOURCE_RESULT_CAP);
    return {
    ...
    const definitions = [
        ...
        },
        {
            ...
            async execute(args, exec) {
                ...
                if (!/^\d{6}$/u.test(code))
                    throw new Error('code must be a 6-digit stock code');
