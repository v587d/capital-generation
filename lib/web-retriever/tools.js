...
export function registerSourceTools(ctx, localFetchOptions) {
    ...
    const definitions = [
        ...
        },
        {
            ...
            description: '取个股研报列表（东财研报库，按 6 位代码；上游只认纯数字——实测带 SH/SZ 前缀返回 0 条，所以 `SH600519` / `600519.SH` 这类写法由本工具先归一化成 6 位再查）。返回日期/标题/机构/研究员/评级/评级变动与该篇研报的当年预测 EPS。⚠️ 列表不含摘要，正文在 PDF 里，而本机直连只处理文本（PDF 会被拒）——**web_retriever 拿不到研报正文**，需要正文请如实说明该边界，不得用标题当结论。券商观点属第三方预测，不是官方事实，回传时标注机构名与发布日期。',
            parameters: jsonObject({
                code: { type: 'string', description: `6 位股票代码${CODE_FORMS}` },
