import { MAX_ITEMS, SourceError, clsTelegraph, cninfoIrm, createGatedEastmoneyTransport, createSourceTransport, eastmoneyFastNews, eastmoneyReports, eastmoneyStockNews, sinaReports, sseinfoQa, thsEpsForecast, wscnLives, } from './sources.js';
...
import { callerSession } from '../tool-exec.js';
import { DatasetStoreError } from '../data-collector/store.js';
import { OcrError } from '../ocr/client.js';
import { locateQuery, parsePageRange, selectPages } from '../ocr/markdown.js';
import { OCR_DOC_ID_PATTERN, buildArtifactMeta, deriveOcrDocId, ocrArtifactRef, readOcrArtifact, readOcrSourceBytes, writeOcrArtifact, OcrArtifactError, } from '../ocr/store.js';
...
function createRetrievalEcho() {
    ...
    return (exec, entry) => {
        ...
        const stored = { ...entry, query: entry.query.slice(0, TRACE_TEXT_CLIP) };
        list.push(stored);
 ...
 */
function throwToolFailure(payload) {
 ...
 * 数组里注册，不另起一个注册函数。
 */
export function registerWebRetrieverTools(ctx, retriever, windClient, ocr) {
 ...
 */
function artifactRepair(sourceKind, source) {
...
function flag(value, name, fallback) {
    ...
    if (typeof value !== 'boolean')
        throw new OcrToolError('INVALID_INPUT', `${name} must be a boolean`);
    return value;
}
 ...
 */
function ocrRequest(args) {
    const present = (key) => args[key] !== undefined && args[key] !== null && args[key] !== '';
    ...
    if (given.includes('job_id')) {
        ...
        if (given.length > 2)
            throw new OcrToolError('INVALID_INPUT', `续查时不要再传 ${given.filter((key) => key !== 'job_id' && key !== 'doc_id').join('/')}`);
        if (pages !== undefined || query !== undefined) {
    ...
    if (given.length > 1)
        throw new OcrToolError('INVALID_INPUT', `一次只走一种形态，收到 ${given.join(' + ')}`);
    const mode = given[0];
    if (mode === 'doc_id') {
        if (refresh || !charts) {
        ...
        if (limit !== undefined && query === undefined)
            throw new OcrToolError('INVALID_INPUT', 'limit 只在 query 形态下有意义的');
...
function pageIndex(document, withHeadings) {
...
function cleanupNote(meta) {
    if (meta.style_chars_removed === 0 && meta.tables === 0)
        return undefined;
    const parts = [];
    if (meta.style_chars_removed > 0)
        parts.push(`已剥掉 ${meta.style_chars_removed} 个排版噪音字符`);
    if (meta.tables > 0)
        parts.push(`${meta.tables} 张 HTML 表已转成 markdown 表格`);
    return parts.join('；');
}
/**
 ...
 */
function doneReceipt(artifact, input) {
    ...
    if (withPageOne && fitsBudget(withPageOne))
        return withPageOne;
    if (fitsBudget(withIndex))
        return withIndex;
 ...
 */
function budgetNote(meta, cleanup, withPageOne) {
...
function pagesReceipt(input) {
    ...
    const envelope = (excerpts, omitted) => ({
    ...
    const notes = [];
    if (!input.explicit)
        notes.push(`没传 pages，默认回第 1 页；全篇 ${meta.pages} 页，用 pages 或 query 继续读`);
    if (omitted.length > 0)
        notes.push(`预算 ${SOURCE_OUTPUT_BUDGET_CHARS} 字符：第 ${omitted.join('/')} 页未放入，单独取回或改用 query`);
    return {
...
function queryReceipt(input) {
    ...
    if (omitted > 0)
        notes.push(`因预算省略其后 ${omitted} 处：调小 limit，或用 pages 取整页`);
    if (hits.length === 0)
        notes.push('没有命中不等于文档里没有这个主题：换关键词，或按 pages 通读');
    return {
        ...metaFields(meta, true),
        ...echoFields(input.echo),
        query: input.query,
...
async function persistAndReceipt(deps, input) {
 ...
 */
function ocrErrorReceipt(error, input) {
...
async function runOcr(deps, trace, args, exec) {
    let requested;
    try {
    ...
    try {
        ...
        if (requested.mode === 'read') {
            const artifact = await readOcrArtifact(deps.store, { session, doc_id: requested.doc_id, signal });
            if (!artifact) {
        ...
        if (requested.mode === 'resume') {
            const docId = requested.doc_id;
            const existing = await readOcrArtifact(deps.store, { session, doc_id: docId, signal });
            if (existing)
                return doneReceipt(existing, { cached: true, echo });
            const outcome = await deps.client.wait(requested.job_id, signal);
        ...
        const target = requested.mode === 'url'
            ? await pdfUrlTarget(requested.url, signal, deps.resolveAddresses)
            : await localFileTarget(deps, requested.file, session, signal);
        ...
        if (!requested.refresh) {
            ...
            if (cached)
                return doneReceipt(cached, { cached: true, echo });
        }
 ...
 */
async function pdfUrlTarget(value, signal, resolveAddresses) {
...
async function localFileTarget(deps, value, session, signal) {
...
function sourceOutput(outcome, recent, tally) {
    const candidates = outcome.items.slice(0, SOURCE_RESULT_CAP);
    const envelope = (items, omitted) => {
        const notes = outcome.note === undefined ? [] : [outcome.note];
        if (omitted > 0) {
            notes.push(`输出预算 ${SOURCE_OUTPUT_BUDGET_CHARS} 字符：本次只放了前 ${items.length} 条，省略其后 ${omitted} 条${outcome.next_cursor === undefined ? '（请缩小 limit 或翻页重查）' : '（用 next_cursor 继续翻页）'}`);
        }
        return {
    ...
    for (const item of candidates) {
        ...
        if (taken > 0 && used + cost > SOURCE_OUTPUT_BUDGET_CHARS)
            break;
        used += cost;
        taken += 1;
    }
 ...
 */
function throwSourceFailure(source, operation, error, recent, tally) {
...
function sourceItemsText(items) {
 ...
 */
export function registerSourceTools(ctx, localFetchOptions) {
    const tools = ctx.get('tools');
    if (!tools)
        return;
    const echo = createRetrievalEcho();
    ...
    async function run(exec, provider, operation, label, produce) {
        const { recent, tally } = trace(exec, provider, `${provider}_${operation}`, label);
        if (localFetchOptions.enabled === false) {
            throwSourceFailure(provider, operation, new SourceError(provider, 'DISABLED', '本机直连已被设置关闭（卡片「允许启动本地提取网页内容」）；本工具全部条目都需本机 HTTP，开关打开后新会话可用'), recent, tally);
        }
        let outcome;
        try {
            ...
            const code = error instanceof SourceError ? error.code : error?.code;
    ...
    const definitions = [
        {
            ...
            async execute(args, exec) {
                ...
                return run(exec, 'cls', 'telegraph', `limit=${limit}`, () => clsTelegraph(transport, { limit, signal: exec.signal }));
            },
        },
        {
            name: 'wscn_lives',
            ...
            async execute(args, exec) {
                ...
                if (!/^[a-z0-9-]+-channel$/u.test(channel))
                    throw new Error("channel must look like 'global-channel' / 'a-stock-channel'");
                const limit = integer(args.limit, 'limit', 1, SOURCE_RESULT_CAP) ?? 20;
                const cursor = args.cursor === undefined ? undefined : requiredString(args.cursor, 'cursor', 64);
                return run(exec, 'wscn', 'lives', `channel=${channel} limit=${limit}`, () => wscnLives(transport, { channel, limit, ...(cursor === undefined ? {} : { cursor }), signal: exec.signal }));
            },
        },
        {
            name: 'cninfo_irm',
            ...
            async execute(args, exec) {
                ...
                return run(exec, 'cninfo_irm', 'questions', `code=${code} page=${pageNum}`, () => cninfoIrm(transport, { code, pageSize, pageNum, signal: exec.signal }));
            },
        },
        {
            name: 'sseinfo_qa',
            ...
            async execute(args, exec) {
                ...
                if (kind !== 'answered' && kind !== 'questions')
                    throw new Error("kind must be 'answered' or 'questions'");
                const page = integer(args.page, 'page', 1, 1000) ?? 1;
                const pageSize = integer(args.page_size, 'page_size', 1, SOURCE_PAGE_SIZE_MAX) ?? 10;
                return run(exec, 'sseinfo', code === undefined ? `market_${kind}` : `company_${kind}`, code === undefined ? `kind=${kind}` : `code=${code} kind=${kind}`, () => sseinfoQa(transport, { ...(code === undefined ? {} : { code }), kind, page, pageSize, signal: exec.signal }));
            },
        },
        {
            name: 'eastmoney_724',
            ...
            async execute(args, exec) {
                const limit = integer(args.limit, 'limit', 1, SOURCE_RESULT_CAP) ?? 20;
                return run(exec, 'eastmoney', 'fast_news', `limit=${limit}`, () => eastmoneyFastNews(eastmoneyClient, { limit, signal: exec.signal }));
            },
        },
        {
            ...
            async execute(args, exec) {
                const code = bareAshareDigits(args.code);
                const limit = integer(args.limit, 'limit', 1, SOURCE_RESULT_CAP) ?? 20;
                return run(exec, 'eastmoney', 'stock_news', `code=${code} limit=${limit}`, () => eastmoneyStockNews(eastmoneyClient, { code, limit, signal: exec.signal }));
            },
        },
        {
            ...
            async execute(args, exec) {
                ...
                return run(exec, 'eastmoney', 'reports', `code=${code} page=${pageNo}`, () => eastmoneyReports(eastmoneyClient, { code, pageSize, pageNo, signal: exec.signal }));
            },
        },
        {
            name: 'sina_reports',
            ...
            async execute(args, exec) {
                const code = args.code === undefined ? undefined : bareAshareDigits(args.code);
                const page = integer(args.page, 'page', 1, 1000) ?? 1;
                return run(exec, 'sina', code === undefined ? 'latest' : 'by_stock', code === undefined ? `page=${page}` : `code=${code} page=${page}`, () => sinaReports(transport, { ...(code === undefined ? {} : { code }), page, signal: exec.signal }));
            },
        },
        {
            ...
            async execute(args, exec) {
                const code = bareAshareDigits(args.code);
                return run(exec, 'ths', 'eps_forecast', `code=${code}`, () => thsEpsForecast(transport, { code, signal: exec.signal }));
            },
        },
    ];
    for (const definition of definitions) {
