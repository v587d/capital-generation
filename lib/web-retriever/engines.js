...
export class AnySearchError extends Error {
    ...
    upstreamCode;
    status;
    constructor(code, path, detail, extra) {
        super(`AnySearch ${path} error: ${detail}`);
        this.name = 'AnySearchError';
        this.code = code;
        if (extra?.upstreamCode !== undefined)
 ...
 */
export function shouldFallbackToLocalFetch(code) {
...
export function createAnySearchClient(baseURL = DEFAULT_BASE_URL, resolveApiKey = async () => envKey('ANYSEARCH_API_KEY')) {
    ...
    async function request(path, body, signal) {
        if (signal?.aborted)
            throw new AnySearchError('ABORTED', path, 'request aborted');
        ...
        const fail = (code, detail, extra) => new AnySearchError(code, path, detail, extra);
        try {
            ...
            try {
                ...
                if (timedOut)
                    throw fail('TIMEOUT', 'request timed out');
                if (signal?.aborted)
                    throw fail('ABORTED', 'request aborted');
            ...
            if (!parsed)
                throw fail('INVALID_RESPONSE', 'invalid JSON response');
