 ...
 * 自包含 HTML 图表的生成器。
 *
 * 这是整个方案的**降级底线**（内部设计文档 docs/design/chart-visualization.md §5.4 的 L2）：
 ...
 */
export function loadVendoredChartLibrary() {
    if (cachedLibrary)
        return cachedLibrary;
    try {
        const fs = nodeFs();
        const source = fs.readFileSync(new URL('vendor/lightweight-charts.standalone.production.js', import.meta.url), 'utf8');
        const manifest = JSON.parse(fs.readFileSync(new URL('vendor/VERSION.json', import.meta.url), 'utf8'));
...
function formatCapturedAt(epochMs) {
    try {
...
export function buildStandaloneHtml(input) {
    ...
    if (meta.downsampled)
        facts.push(`已从 ${meta.original_points} 点下采样（保留极值形状，非全量）`);
    if (meta.dataset_id)
        facts.push(`dataset ${meta.dataset_id}`);
    if (meta.source_label)
        facts.push(meta.source_label);
    if (meta.captured_at !== undefined)
        facts.push(`数据截至 ${formatCapturedAt(meta.captured_at)}`);
    if (meta.time_field)
        facts.push(`时间字段 ${meta.time_field}`);
    if (meta.axis === 'index')
        facts.push('分类轴（按行序）');
