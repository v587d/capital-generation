 ...
 * 插件带的同库不同版本互相覆盖的来源（见 docs/design/chart-visualization.md §9 风险表）。
  ...
  function renderCapitalChart(container, payload, library) {
    ...
    if (Array.isArray(payload.series)) {
      for (var index = 0; index < payload.series.length; index += 1) {
        ...
        var color = index === 0 ? colors.brand : PALETTE[(index - 1) % PALETTE.length]
        var options = { title: item.label || '', priceLineVisible: false, lastValueVisible: true }
        var series = null
        if (item.type === 'area') {
          options.lineColor = color
          options.topColor = color + '33'
          options.bottomColor = color + '05'
          options.lineWidth = 2
