 ...
 * 监听改写。
 */
import { agentSessionId, isCapitalAgent } from './root-tool-policy.js';
/** bash 工具名（`@deepseek-ai/dsh-tool-bash` 注册的全局名）。 */
export const BASH_TOOL_NAME = 'bash';
/** A 结构层拒绝文案：模型可据此改走委派。 */
export const BASH_DELEGATED_ONLY = 'bash 只对被委派的数据子 Agent 开放，当前调用方不是被委派的子会话；数据与统计请由主 Agent 委派 data_collector / data_junior。';
/** B UX 层拒绝文案：说清"这不是用户拒绝"，并给出正确路径。 */
export const BASH_ESCALATION_UNREACHABLE = 'bash 的沙箱升级在子 Agent 会话里永远无法获批（框架把委派会话的审批策略固定为 never，不会去问用户），不要重试：'
 ...
 * 绝不解析 `command`——理由见文件头。
 */
export function bashGuardReason(exec) {
    if (exec?.name !== BASH_TOOL_NAME)
        return undefined;
    const parent = exec.agent?.session?.header?.parentSession;
    if (typeof parent !== 'string' || parent.length === 0)
        return BASH_DELEGATED_ONLY;
...
function installAgentGuard(agent) {
    let tools;
    try {
 ...
 */
export function registerBashGuard(ctx) {
