 ...
 */
function requestParamsError(error, capability) {
    if (errorCode(error))
        return undefined;
    const message = error instanceof Error ? error.message : String(error);
    if (!/(?:\bparameter\b|\bparams\b|missing required parameter|unsupported parameter|at least one of)/i.test(message))
        return undefined;
    ...
    failure.code = 'request_params_invalid';
    return failure;
}
...
export function registerDataCollectorTools(ctx, hub, diagnostics) {
    ...
    if (!tools)
        return;
    const registrations = [
        tool('request_data', '向数据管道请求数据：宿主先按 capability+params 校验当前 session 的已验证 manifest；force_refresh=false 或省略且命中未过期 Dataset 时直接返回已有 DatasetRef，不访问上游；force_refresh=true 才重新取数并生成新的不可变 Dataset（默认保留 7 天），绝不覆盖旧 Dataset。本工具只返回 DatasetRef（dataset_id / artifact_ref / schema / row_count / captured_at / retention_until 等元数据），绝不返回原始数据行。capability 必须是 list_capabilities 返回的短能力名（如 ticker_search / quote / history / trading_calendar），params 必须按 describe_capability({ capability }) 返回的 input_schema 填写。若返回 request_params_invalid，先重新调用 describe_capability({ capability: "<当前能力名>" })，再按 schema 修正并重试一次；不要凭记忆猜参数。task_id 必须由主 Agent 在本轮委派前确定，并在所有子 Agent 与最终报告中原样复用；不要由 data_collector 自行生成。相同 capability+params 的并发请求合并为同一次执行。当前 workspace 只读或落盘失败时抛错 workspace_not_writable（绝不退化为内存假成功）。请求归属自动记为当前 delegated child session，不接收 requester_agent_id 参数。', jsonObject({
            capability: { type: 'string' },
            params: freeObject,
            force_refresh: { type: 'boolean' },
            task_id: { type: 'string' },
        }, ['capability', 'params', 'task_id']), datasetRefSchema(), async (args, exec) => {
            ...
            try {
                ...
                return await hub.request({
                    ...
                    task_id: boundedString(args.task_id, 'task_id', MAX_ID_LENGTH),
                    session,
                }, { signal: exec.signal });
            }
            catch (error) {
        ...
        tool('describe_capability', '读取**一个**能力的完整契约：description（单位、null 语义、时间与分页口径）、input_schema（params 的取值与约束）、output_schema（返回字段）与 paginated。request_data 的 params 必须按这里返回的 input_schema 填写；request_data 若返回 request_params_invalid，应回到这里重新读取后再重试。一个能力描述一次即可：结果已在本 session 历史里，重复描述同一个能力只会白占上下文；需要几个能力就分别调用几次，不要用逗号把多个名字塞进一次调用。错误按 code 引导：capability_required（没给名字，先 list_capabilities()）、capability_invalid（不是字符串/超长/含逗号，改为目录中的单个名字）、capability_catalog_empty（数据源未注册，别重试，回告主 Agent）、capability_unknown（名字不存在，错误里会列出全部可用能力名并要求原样复制）。', jsonObject({ capability: { type: 'string' } }, ['capability']), jsonObject({
            capability: { type: 'string' },
            summary: { type: 'string' },
            paginated: { type: 'boolean' },
