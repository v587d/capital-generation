 ...
 */
function buildTimeFacts(rows, timeColumn, orderedAscending, numericColumns) {
    const project = (row) => {
        const projected = {};
        if (!isRecord(row))
            return projected;
...
function validateProfile(value) {
    ...
    if (value.time_facts !== undefined) {
        ...
        if (Object.keys(item).some((key) => !['time_column', 'ordered_ascending', 'covered_from', 'covered_to', 'first', 'last'].includes(key)))
