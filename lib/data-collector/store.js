...
export class WorkspaceDatasetStore {
    ...
    async readDatasetSlice(datasetId, session, input = {}, signal) {
        const ref = await this.requireRef(datasetId, session, signal);
        if (ref.format !== 'json_rows')
            throw new DatasetStoreError('dataset_format_unsupported', `Dataset format ${ref.format} cannot be sliced`);
        const offset = input.offset ?? 0;
        const limit = input.limit ?? DEFAULT_SLICE_ROWS;
        if (!validNonNegativeInteger(offset))
            throw new DatasetStoreError('dataset_slice_out_of_range', 'offset must be a non-negative integer');
        if (!Number.isSafeInteger(limit) || limit < 1 || limit > MAX_SLICE_ROWS)
            throw new DatasetStoreError('dataset_slice_out_of_range', `limit must be between 1 and ${MAX_SLICE_ROWS}`);
        const raw = await this.readRaw(ref, session, signal);
        if (offset > raw.rows.length)
            throw new DatasetStoreError('dataset_slice_out_of_range', `offset ${offset} exceeds total row count ${raw.rows.length}`);
        ...
        if (input.columns !== undefined) {
            ...
            for (const column of columns)
                if (!available.has(column))
                    throw new DatasetStoreError('dataset_column_not_found', `column ${column} is not present in Dataset`);
            rows = rows.map((row) => {
                if (!isRecord(row))
                    throw new DatasetStoreError('dataset_column_not_found', 'columns projection requires object rows');
                const projected = {};
                for (const column of columns)
                    if (column in row)
        ...
        if (byteLength(encoded) > MAX_SLICE_OUTPUT_BYTES)
            throw new DatasetStoreError('dataset_too_large_for_slice', `slice output exceeds ${MAX_SLICE_OUTPUT_BYTES} bytes`);
        return {
            dataset_id: datasetId,
            offset,
            limit,
    ...
    async profileDataset(input) {
        ...
        if (ref.format !== 'json_rows')
            throw new DatasetStoreError('dataset_format_unsupported', `Dataset format ${ref.format} cannot be profiled`);
    ...
    async readRaw(ref, session, signal) {
        const root = this.readRoot(session);
        if (!root || !this.fs)
            throw new DatasetStoreError('filesystem_unavailable', 'workspace filesystem is unavailable');
        ...
        if (typeof info?.size === 'number' && info.size > MAX_RAW_READ_BYTES)
            throw new DatasetStoreError('dataset_too_large_for_slice', `raw Dataset exceeds ${MAX_RAW_READ_BYTES} bytes`);
        let text;
        try {
            text = await this.fs.readText(target, signal);
        }
        ...
        if (isRecord(value) && Array.isArray(value.item))
            return { rows: value.item, shape: 'envelope_item' };
...
function buildProfile(rows, requestedTimeColumn, primaryKey) {
...
function validateColumns(value, errorCode = 'dataset_column_not_found') {
    if (!Array.isArray(value) || value.length > 64)
        throw new DatasetStoreError(errorCode, 'columns must contain at most 64 names');
...
function validateProfile(value) {
    ...
    if (value.statistics !== undefined) {
        if (!isRecord(value.statistics) || Object.keys(value.statistics).length > 64)
            throw new DatasetStoreError('profile_invalid', 'statistics must contain at most 64 columns');
        statistics = {};
