...
| L15 | settings / credentials 扩展面：host `settings.register(ns, schema, { base, applies })`（`applies: 'live' \| 'restart'`）；客户端 `ctx.settingsScope.bind({ namespace })`（`SettingsScopeSpec.namespace`）；Slot `settings.plugin.item`（`kind: 'keyed'`，键 = 命名空间）；`ctx.remote.credentials.describe/set` + 转发的 `credentials/reference-updated` / `settings/document-updated` | `capital-config` 是 host 平面 settings 卡片：host 半边注册 `capital-generation` 命名空间（**卡片座位 + ref 名来源**），浏览器半边据命名空间读 ref 名、把密钥写进 credentials 域；主插件读同一命名空间覆盖自身配置 | 在 `dsh-settings/lib/types/index.d.ts` 断言 `settings: SettingsProvider`、`register(...)` 签名、`SettingsApplies`、`SettingsRegisterOptions.{base,applies}`；在 `dsh-client-ui-settings` 断言 `SettingsScopeSpec.namespace` 与 `SettingsScopeBinder.bind`；在 `dsh-client-ui-settings-plugins` 断言 `settings.plugin.item` 仍 `kind: 'keyed'`；在 `dsh-api-remotes` 断言两个事件仍在转发白名单、`credentials/describe`+`credentials/set` 仍在；探针见 `check-dsh` L15。本仓回归见 `test/capital-config.test.mjs`（host `SETTINGS_NAMESPACE` ≡ 客户端 `NS`；两处 schema 默认值一致） | **静默**：命名空间不再被服务或卡片键不再匹配 → 卡片在设置页消失（零报错，用户以为"没有这个插件"）；credentials remote/事件改名 → 密钥写不进去、状态徽标不刷新 | `capital-config/index.js`（`SETTINGS_NAMESPACE`）+ `client.src.cjs`（`NS`，两者必须相等）；`src/index.ts` 的 settings 消费 |
