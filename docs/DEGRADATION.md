...
## LLM-first 错误消息规范 (v0.3.0 确立, 实施审计见 PLAN-0.3.0.md §9.1)

**定位**: MCP 的消费方是 LLM, 不是人 — **每条错误/引导消息都是给 LLM 的下一条指令**。
这是本项目区别于同花顺/万得原生 SDK 的核心优势: 不只转发 vendor 错误, 而是翻译成
LLM 可执行的动作。判定表"模型看到"列是本规范的最初形态, 本节把它升级为硬性要求。

**每条消息三要素 (缺一不可)**:
1. **发生了什么** — vendor + endpoint + code/类型; vendor 原文细节保留 (如 request_id),
   但不允许原文透传作为消息主体 (LLM 无法据此决策)
2. **为什么** — 归类: 限流/权限/参数/暂无数据/网络/内部 (与 FinError.kind 一致)
3. **下一步动作** — LLM 可执行: 等多久重试 / 改什么参数 / 换什么标的 / 检查什么配置

**已知反例 (必须修, 2026-08-15 实测)**: THS `2003 "Invalid or revoked API key"` 原文透传
— 该码在实测中**可能是临时限流而非 key 失效** (LESSONS §6.5), 原文会让 LLM 误判并引导
用户换 key (错误动作); 改写为"同花顺临时拒绝 (2003): 可能是高频请求触发限流,
请稍后重试; 若持续出现再检查 key" 类措辞。

**适用范围**: 所有 `FinError` message、信封 warnings、非错误引导返回
(ambiguity / not_found / 资产类型不符 / 降级说明) 一视同仁。
新增/修改消息时按三要素自查; 全量审计清单见 PLAN-0.3.0.md §9.1。
