...
| `client.src.cjs` | 浏览器半边**源码**：注册 `tool.call.toolview['render_chart']`，取数 → 渲染 → “放大”开自包含 HTML |
...
设计（`docs/design/chart-visualization.md` §9）要求客户端**不产生页面全局**，所以
...
工厂函数，断言它按工具名注册 toolview、库已内联、react 保持 external、不依赖页面全局。
...
本包对 DSH 的依赖只有两项，均登记在 `docs/reference/dsh-surface-ledger.md`：
`ctx.webServer.register`（L6）与 `dsh.client` / `exports["./client"]`（L1、L2）。
`npm run check:dsh` 会逐条断言。
